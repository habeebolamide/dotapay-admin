...
interface EditAutoSettlementConfiguration {
  isOpen: boolean;
  onClose: () => void;
  autosettleMent: AutoSettlement;
  onEditAutoSettlement: (data: CreateAutoSettlementConfigurationRequest) => Promise<void>;
}

export function EditAutoSettlementConfiguration({ onEditAutoSettlement,autosettleMent,isOpen, onClose, }: EditAutoSettlementConfiguration) {
  ...
  const form = useForm<FormData>({
    ...
    defaultValues: {
      wallet_id: autosettleMent.wallet.slug || '',
      settlement_bank_id: autosettleMent.settlement_bank.code || '',
      minimum: formatKoboToNaira(autosettleMent.minimum).replace('₦', '').replace(',', '') || '',
      frequency: autosettleMent.frequency_label || '',
    },
  });
  ...
  const onSubmit = async (data: FormData) => {
    setLoading(true);
    try {
      await onEditAutoSettlement(data);
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      ...
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>Edit Auto Settlement Configuration</DialogTitle>
          <DialogDescription>
            Edit an existing auto settlement bank configuration.
          </DialogDescription>
        </DialogHeader>
          ...
          >
                    ...
                    <Select onValueChange={field.onChange} value={field.value}>
                      ...
                      <SelectContent>
                        {loadingSettlementBank ? (
                          ...
                          settlementBanks.map((settlement) => (
                            <SelectItem key={settlement.id} value={settlement.code}>
                              {settlement.name}
                            </SelectItem>
                          ))
                        ) : (
            ...
            <DialogFooter>
              ...
              >
                ...
                Edit AutoSettlement Configuration
              </Button>
            </DialogFooter>
          </form>
        </Form>
